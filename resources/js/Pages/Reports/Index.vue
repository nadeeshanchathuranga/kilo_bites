          ...
          <tbody class="text-[12px] font-medium">
            <tr v-for="(s, i) in sales" :key="s.id ?? i" class="border-b transition duration-200 hover:bg-gray-100">
              ...
              <td class="p-3 num text-center">
                <span v-if="ownerDiscountAmount(s) > 0">
                  {{ toMoney(ownerDiscountAmount(s)) }}
                  <span v-if="ownerDiscountPercent(s) > 0"> ({{ ownerDiscountPercent(s).toFixed(2) }}%)</span>
                </span>
                <span v-else>—</span>
              </td>
              <td class="p-3 num text-center">
...
// ---------- NEW: owner discount helpers ----------
const ownerDiscountAmount = (s) => {
  const raw = Number(s.owner_discount_value || 0);
  // If owner discount is stored as a percent type, calculate it from priceWithService
  if (s.owner_discount_type === "percent") {
    return (priceWithService(s) * raw) / 100;
  }
  // otherwise treat as absolute amount
  return raw;
};

const ownerDiscountPercent = (s) => {
  const base = priceWithService(s) || 0;
  if (!base) return 0;
  return (ownerDiscountAmount(s) / base) * 100;
};
// ---------- end owner helpers ----------
...
const saleProfit = (s) => {
  ...
  const ownerDisc = ownerDiscountAmount(s);
...
const salesOwnerDiscountTotal = computed(() =>
  (sales.value || []).reduce((sum, s) => sum + ownerDiscountAmount(s), 0) // <-- use helper
);
...
const wrongOwnerDiscount = computed(() => (wrongSales.value || []).reduce((a, s) => a + ownerDiscountAmount(s), 0));
const wrongProfit = computed(() =>
  (wrongSales.value || []).reduce((sum, s) => sum + saleProfit(s), 0)
);
...
const downloadSalesTableExcel = () => {
  ...
  const rows = (sales.value || []).map((s, i) => {
    ...
    const owner = ownerDiscountAmount(s) > 0 ? (s.owner?.name ?? "—") : "—";
    const ownerDisc = ownerDiscountAmount(s);
    const profit = saleProfit(s); // <-- updated
...
const downloadWrongBillsExcel = () => {
  ...
  const rows = (wrongSales.value || []).map((s, i) => {
    ...
    return [
      ...
      owner,
      ownerDiscountAmount(s),
      saleProfit(s), // <-- updated
      "Wrong",
    ];
  });
...
const downloadWrongBillsCSV = () => {
  ...
  const rows = (wrongSales.value || []).map((s, i) => [
    ...
    ownerDiscountAmount(s) > 0 ? (s.owner?.name ?? "—") : "—",
    (+ownerDiscountAmount(s) || 0).toFixed(2),
    (saleProfit(s) || 0).toFixed(2), // <-- updated
    "Wrong",
  ]);
